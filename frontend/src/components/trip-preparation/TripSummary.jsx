...
function Row({ label, value, placeholder = "—" }) {
  return (
    <div className="flex items-start justify-between gap-3 py-2.5 border-b border-slate-100 last:border-b-0">
      <span className="text-xs font-body uppercase tracking-wide text-slate-400 pt-0.5">
        {label}
      </span>
...
export default function TripSummary({
  ...
  return (
    <div className="card-elevation p-6 sticky top-6">
      ...
      <Row
        label="Distance"
        value={validationResult ? `${validationResult.distance_km} km` : null}
      />
      <Row
        label="Travel Mode"
        value={preferredMode ? MODE_LABELS[preferredMode] : null}
